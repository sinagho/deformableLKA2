...
from networks.MaxViT_deform_LKA import MaxViT_LargeKernelDualTransformer
...
snapshot_path = "./model_results2017_MSA2Net/{}/{}".format(args.exp, 'model')
...
Net = MaxViT_LargeKernelDualTransformer(img_size=args.img_size, num_classes=1).to(device)

num_classes = 1
optimizer = optim.SGD(Net.parameters(), lr=args.base_lr, momentum=0.9, weight_decay=0.0001)
scheduler = optim.lr_scheduler.ReduceLROnPlateau(optimizer, 'min', factor=0.5, patience=10)

####### The initial Losses have been replaced with Boundary Difference Over Union Loss ######

# criteria  = torch.nn.BCELoss()
#criteria = torch.nn.BCEWithLogitsLoss()
criteria = BoundaryDoULoss(num_classes)
